...
function App() {
  ...
  */ 
  //App()  랜더링 (호출될때 마다) 함수 (someFunction) 는 바꿘다

  //someFunction 함수의 주소가 가지고 있다고 number state 바뀌면 그 때 함수가 호출
  const someFunction = useCallback(()=>{
      console.log("number : " + number);
      return;
  },[number]); //의존성 배열 number 값이 바뀌면 새로운 함수 생성(새로운 주소를 가지는 함수)


  useEffect(()=>{
      console.log("useEffect 동작하네요");
  },[someFunction]);
  //함수도 주소값을 가지고 있다 (함수도 객체)
  //함수의 주소가 바뀌면 useEffect 실행
 //useEffect [값] 바뀌면 호출 
 /*
