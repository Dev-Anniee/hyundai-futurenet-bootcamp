...
--------------------------------------------------------------------------------
-- cursor , procedure(목표) , function , Trigger 고급자원 (메모리, CPU)
...
as
select * from emp where 1=2; --빈껍데기 테이블 생성
...
create or replace procedure proc_calculate_salary --프로시져 장점 input,output parameter 다루기 쉬워진다
    is
    cursor sal_cursor is
        select empno, ename, job_type , monthly, hours , hourly, meal
        from salary_info;
...
--스케쥴 (JOB) : 매월 말일 12시에 실행 되게 예약해 놓으면 효율적이다
--------------------------------------------------------------------------------
/*
...
4조 할인율을 적용하는 테이블있고 ..... 상품테이블 상품 할일율을 적용 매주 금요일
*/
--------------------------------------------------------------------------------
--05.01 오전 수업 끝
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK; --당장은 의미가 없다 (여러 개의 DML 실행 의미가 있다)
        v_err_code := SQLCODE;
        v_err_msg := SQLERRM;
...
--MS (c# , vb.net - ms-sql) : 모든 90% 으로 프로시져 ..
--JAVA 활용할 수 있는 프레임워크
...
select * from USER_CONSTRAINTS where table_name='USP_EMP';
...
create or replace trigger tri_02
    after update on tri_emp -- tri emp 테이블에 update 되고 난 후 트리거 동작
BEGIN
...
-- insert update delete 가 일어나면 자동으로 실행되는 프로시져를 만들겠다
    -- 1. before 트리거
    -- 2. after 트리거
-----------------------------------------------------------------------------
--예제1)
...
create or replace trigger emp_audit_tr
    after insert or update or delete on emp2 -- 여러 상황 케이스로 나눌 수도 있다
    --for each row
begin
    if inserting then
...
create or replace trigger emp_audit_tr
    ...
    for each row --중요, 각 건의 처리가 가능, 반영된 수 만큼
begin
    if inserting then
        insert into emp_audit
        values(emp_audit_tr.nextval, user, 'inserting', sysdate);
...
--OLD , NEW
--내부적으로 가상 테이블 : old, new
--INSERTED , DELETED 다른 DB / 업데이트는 필요없다 -> old -> new

drop table emp_audit;

create table emp_audit ( --추적
