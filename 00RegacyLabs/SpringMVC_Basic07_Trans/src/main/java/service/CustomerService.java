			...
			try {
				/*
				 begin 
					...
					noitceDao.updateOfMemberPoint("admin")
				 end
				 결과) 둘다 commit or 둘다 rollback
				 
				 1. @Transactional > 동작 > <tx:annotation-driven  설정이 전제 조건
				 2. 둘다 성공(commit) , 둘 중에 하나만 실패(rollback)
				 3. default > 예외가 발생하면 > rollback 시나리오로
				 4. transaction-manager="transactionManager"/ 감시하고 있다가 예외가 발생하면 (모든 DML rollback)
				 
				*/	
					NoticeDao noticeDao = sqlSession.getMapper(NoticeDao.class); //추가
				    //2개의 업무가 실행
					noticeDao.insert(n);  //DB insert
				    noticeDao.updateOfMemberPoint("admin"); //원칙은 (session 에서 저장된 userid)
			
				    //catch 타지 않고
				    //여기까지 오면
				    System.out.println("정상 insert ,,, 정상 update 둘다 자동 commit");
			} catch (Exception e) {
				
				System.out.println("Transaction 예외 발생 : " + e.getMessage());
				//둘중에 하나라도 문제가 생기면
				throw e;
				//예외 발생에 대한 처리는 > controller 가 
			} 
