...
>> select distinct job  from emp where sal > 3000 and job = "IT"
>> RDMS (오라클, mysql, mssql 소프트웨어가 최적화) 데이터 가공
...
public class Ex05_Stream {
	...
	public static void main(String[] args) {
		...
		//1. 데이터가 지나가는 통로 생성
		Stream<String> stream = Stream.of(strarr); // 통로 만들고
		
		
		//2. 중간연산 (가공하기)
		//2.1 문자열 길이가 2 이하인 것만 필터링
		stream = stream.filter(s -> s.length() <= 2);
		//2.2 중복제거
		stream = stream.distinct(); //중보제거
		//3 알파벳 순으로 정렬 (대소문자 무시)
		...
		List<String> myList = Arrays.asList("a1" , "a2" , "b1" , "b2" , "c2" , "c1");
		 ...
		 long count2 = 	myList.stream()  //생성하기
					   .filter(s -> s.startsWith("c")) //가공
					   .map(String::toUpperCase) //가공 메서드 참조 대문자
			           .sorted()  //정렬 중간연산
		...
		.filter(s -> s.startsWith("c"))  //중간연산
		.map(String::toUpperCase) //중간연산
		.sorted() //중간연산
		.peek(System.out::println) //중간연산
		.collect(Collectors.toList()); //최종결과 소비
